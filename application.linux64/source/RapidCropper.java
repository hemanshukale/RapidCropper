...
public class RapidCropper extends PApplet {
...
public void draw() {
  ...
  int mx = mouseX + PApplet.parseInt(kx), my = mouseY + PApplet.parseInt(ky);
  ...
  {
  if (sh) multi += 0.04f; // at a constant speed
  else    multi += 0.05f*multi/2; // speed proportional to size of rectangle
  }
  ...
  {
   if (sh)  denf += 0.2f; // when shift is pressed, increases denominator, decreasing aspect ratio 
   else     numf += 0.2f; // when shift is not pressed, increases numerator, decreasing aspect ratio
  }
  if (mi)
  {
   if (sh)  denf -= 0.2f; // when shift is pressed, decreases denominator, decreasing aspect ratio
   else     numf -= 0.2f; // when shift is not pressed, decreases numerator, decreasing aspect ratio
  }
  num = (int) (numf + 0.5f);
  den = (int) (denf + 0.5f); 
...
public String getstr(int number) // convert the count into string for saving of cropped images
{
  String numm = "";  
...
public String[] listFileNames(String dir) { //lists file names
...
public boolean isImage( String nam) //checks if the file is an image by looking at extensions
{
  boolean ret = false;
...
public void keyReleased()
 ...
 case SHIFT : {sh = false;} break; case CONTROL : {con = false;} break;
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "RapidCropper" };
    if (passedArgs != null) {
